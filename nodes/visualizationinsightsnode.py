...
class InsightAgent:
    ...
    def _init_llms(self):
        ...
        try:
            print("🧠 Initializing Gemma-2-9B-IT for analytical insights...")
            self.llm = GemmaLLM(
            ...
            if self.llm.is_available():
                ...
                print("⚠️ Gemma LLM not available. Limited analytical capabilities.")
                self.llm = None
        ...
        try:
            print("⚡ Initializing Groq GPT-OSS-120B for ultra-fast calculations...")
            ...
            if self.groq_llm.is_available():
                ...
                print("⚠️ Groq LLM not available. Will use standard calculation methods.")
                self.groq_llm = None
                
        except Exception as e:
            print(f"⚠️ Failed to init Groq LLM ({e}).")
            self.groq_llm = None
    ...
    def _perform_groq_analysis(self, df: pd.DataFrame, question: str, classification, numeric_cols: list, categorical_cols: list) -> dict:
        ...
        try:
            print(f"⚡ Using Groq GPT-OSS-120B for {classification.intent.value} analysis...")
  ...
  "group_by": "optional_column_for_grouping"
            ...
            if groq_response and len(groq_response.strip()) > 100:
                ...
                if viz_html:
                    ...
                    enhanced_answer = f"⚡ **Ultra-Fast Statistical Analysis (Groq GPT-OSS-120B)**\n\n{groq_response}\n\n---\n\n📊 **Visualization Analysis**\n\n{viz_message}"
                else:
                    enhanced_answer = f"⚡ **Ultra-Fast Statistical Analysis (Groq GPT-OSS-120B)**\n\n{groq_response}"
                ...
                print(f"🔄 Groq analysis insufficient, using Gemma fallback...")
                return self._perform_gemma_analysis(df, question, classification, numeric_cols, categorical_cols)
    ...
    def _perform_gemma_analysis(self, df: pd.DataFrame, question: str, classification, numeric_cols: list, categorical_cols: list) -> dict:
        ...
        try:
            print(f"🧠 Using Gemma-2-9B-IT for {classification.intent.value} analysis...")
